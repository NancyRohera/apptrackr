...
function ApplicationModal({ onClose, onSave, existingApp, darkMode }) {
  ...
  function validate() {
    ...
    } else if (form.company.length > 50) {
      newErrors.company = "Company name must be under 50 characters"
    }

    if (form.role.length > 80) {
      newErrors.role = "Role must be under 80 characters"
    }

    if (form.secondPreference.length > 80) {
      newErrors.secondPreference = "Second preference must be under 80 characters"
    }
    ...
    if (form.dateApplied) {
      ...
      if (isNaN(date.getTime())) {
        ...
        newErrors.dateApplied = "Date applied can't be in the future"
      }
    }
    ...
    if (form.interviewDate) {
      ...
      if (isNaN(date.getTime())) {
        newErrors.interviewDate = "Invalid date format"
      }
      if (form.dateApplied && form.interviewDate < form.dateApplied) {
        newErrors.interviewDate = "Interview date can't be before date applied"
      }
    }
    ...
    if (form.notes.length > 300) {
      newErrors.notes = `Notes too long — ${form.notes.length}/300 characters`
    }
  ...
  const errorInputClass = darkMode
    ? "bg-red-950 border-red-500 text-white placeholder-gray-500"
      ...
      <div className={`${modalBg} rounded-2xl shadow-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto`}>
        <div className="flex items-center justify-between mb-5">
          <h2 className={`text-lg font-bold ${titleColor}`}>
            {existingApp ? "Edit Application" : "Add Application"}
          </h2>
            ...
            <select
              ...
              onChange={e => {
                if (e.target.value === "Other") {
                  setForm(prev => ({ ...prev, source: "other_custom" }))
                } else {
        ...
        <div className="flex gap-3 mt-6">
          ...
          >
            {existingApp ? "Save Changes" : "Add Application"}
          </button>
        </div>
      </div>
    </div>
